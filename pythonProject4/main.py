...
def get_ticker(coin1="btc", coin2="usd"):
    #response = requests.get(url="https://yobit.net/api/3/ticker/eth_btc-eth_usdt?ignore_invalid=1")
...
def get_depth(coin1="btc", coin2="usd", limit=150):
    response = requests.get(url=f"https://yobit.net/api/3/depth/{coin1}_{coin2}?limin={limit}&ignore_invalid=1")
...
def get_trades(coin1="btc", coin2="usd", limit=150):
    response = requests.get(url=f"https://yobit.net/api/3/trades/{coin1}_{coin2}?limin={limit}&ignore_invalid=1")
